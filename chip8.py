...
class cpu(pyglet.window.Window):
    ...
    def initialize(self): 
        ...
        self.set_size(64 * 10, 32 * 10)
        self.set_caption("CHIP-8 Emulator")
        ...
        # Change from 10x10 to match your new scale
        pixel_data = b'\xff\xff\xff\xff' * (8 * 8)  # 15x15 white pixel
        self.pixel = pyglet.image.ImageData(8, 8, 'RGBA', pixel_data)
    ...
    def _8ZZZ(self):
        ...
        try:
            ...
            print(f"unknown instruction: {self.opcode:04x}")
    ...
    def _FZ0A(self):
        ...
        # if self.key_wait and Symbol in KEY_MAP:
        #     self.gpio[self.wait_register] = KEY_MAP[symbol]
        #     self.key_wait = False
    def _FZ15(self):
    ...
    def _FZ29(self):
        ...
        self.index = 0x50 + (self.gpio[self.vx] * 5)
    ...
    def on_key_press(self, symbol, modifiers):
        if symbol in KEY_MAP:
            key = KEY_MAP[symbol]
            self.key_inputs[key] = 1

            if self.key_wait:
                self.gpio[self.wait_register] = key
                self.key_wait = False
    ...
    def main(self):
        ...
        pyglet.clock.schedule_interval(self.step, 1/500)
        pyglet.clock.schedule_interval(self.update_timers, 1/60)
        pyglet.app.run()
